...
namespace CoreProjectRole.Controllers
{
    public class MyHomeController : Controller
    {
            ...
            public MyHomeController(ApplicationDbContext context, IWebHostEnvironment hostEnvironment)
            {
                _context = context;
            ...
            public IActionResult Index(int? id)
            {
                ...
                var categoryWiseProductQty = from p in ctx.ProductA
                ...
                var listProduct = (from p in ctx.ProductA
            ...
            public async Task<IActionResult> Create(Category model, string[] ProductName, decimal[] Price, int[] Quantity, DateTime[] ExpireDate, IFormFile[] imgFile)
            {
                ...
                var listProduct = new List<ProductSPA>();
                ...
                ctx.ProductA.AddRange(listProduct);
                ctx.SaveChanges();
            ...
            {
                var ctx = _context;
                var oProduct = (from p in ctx.ProductA
                                join c in ctx.Categories on p.CategoryId equals c.CategoryId
            ...
            public async Task<IActionResult> Edit(VmProduct model)
            {
                ...
                var oProduct = ctx.ProductA.Where(w => w.ProductId == model.ProductId).FirstOrDefault();
                if (oProduct != null)
                {
            ...
            public async Task<IActionResult> EditMultiple(Category model, int[] ProductId, string[] ProductName, decimal[] Price, int[] Quantity, DateTime[] ExpireDate, IFormFile[] imgFile)
            {
                ...
                for (int i = 0; i < ProductName.Length; i++)
                {
                    if (ProductId[i] > 0)
                    {
                        ...
                        int pid = ProductId[i];
                        var oProduct = ctx.ProductA.Where(w => w.ProductId == pid).FirstOrDefault();
                        if (oProduct != null)
                        {
            ...
            {
                var ctx = _context;
                var oProduct = ctx.ProductA.Where(p => p.ProductId == id).FirstOrDefault();
                if (oProduct != null)
                {
                    ctx.ProductA.Remove(oProduct);
                    ctx.SaveChanges();
            ...
            {
                var ctx = _context;
                var listProduct = ctx.ProductA.Where(p => p.CategoryId == id).ToList();
