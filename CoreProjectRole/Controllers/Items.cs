...
namespace CoreProjectRole.Controllers
{
    public class Items : Controller
    {
        ...
        public ActionResult Create(long? id)
        {
            ...
            {
                ViewBag.CategoryID = new SelectList(_context.CategorieA, "ID", "Name", id);
            }
            return PartialView();
        }
        ...
        public async Task<ActionResult> Create([Bind("ID,Name,Description,EntryDate,Quantity,CategoryID")] ItemA item, IFormFile file)
        {
            //string path = "";
            if (ModelState.IsValid)
            {
                if (file != null)
                {
                    ...
                    return RedirectToAction("Index", "CategorieA");
                }
                else
                {
                    ViewBag.CategoryID = new SelectList(_context.CategorieA, "ID", "Name", item.CategoryID);
        ...
        public async Task<ActionResult> Edit(long? id)
        {
            ...
            ItemA item = await _context.Items.FindAsync(id);
            if (item == null)
            {
                //return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind("ID,Name,Description,Image,EntryDate,Quantity,CategoryID")] ItemA item, IFormFile file)
        {
            string path = "";
            if (ModelState.IsValid)
            {
