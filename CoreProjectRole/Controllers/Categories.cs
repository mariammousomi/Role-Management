...
namespace CoreProjectRole.Controllers
{
    ...
    public class Categories : Controller
    {
        ...
        {
            ViewBag.CategoryID = new SelectList(_context.CategorieA, "ID", "Name");
            return View(await _context.CategorieA.ToListAsync());
        }
        ...
        public ActionResult GetCategoryWiseItems(long? id)
        {
            ...
            ViewData["id"] = id;
            List<ItemA> items = _context.Items.Where(e => e.CategoryID == id).ToList();
        ...
        public ActionResult Create([Bind("ID,Name,Items")] CategoryA category, IFormFile[] Image)
        {
            //string path = "";
            try
            {
                if (ModelState.IsValid)
                {
                    if (Image != null)
                    {
                        ...
                        _context.CategorieA.Add(category);
        ...
        public async Task<ActionResult> Edit(long? id)
        {
            ...
            ViewBag.CategoryID = new SelectList(_context.CategorieA.Include("Items"), "ID", "Name", id);
            CategoryA category = await _context.CategorieA.Include(e => e.Items).SingleAsync(e => e.ID == id);
            if (category == null)
            {
                //return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind("ID,Name,Items")] CategoryA category, IFormFile[] file)
        {
            string path = "";
            if (ModelState.IsValid)
            {
                ...
                foreach (ItemA item in category.Items)
                {
        ...
        {
            CategoryA category = await _context.CategorieA.FindAsync(id);
            _context.CategorieA.Remove(category);
            await _context.SaveChangesAsync();
            return RedirectToAction("Index");
        }
